...
# from superset.iou import scheduler

from superset.iou.views import (
    CompanyBrandView,
    FulfillmentActionView,
    FulfillmentActionTypeView,
    FulfillmentActionTypeAttributeView,
    FulfillmentActionValueView,
    LoanView,
    LoanChannelView,
    LoanTransactionView,
    LoanTransactionTypeView,
    LoanTypeView,
    LoanTypeLimitView,
    NotifyPersonView,
    NotifyTypeView,
...
class Superset(BaseSupersetView):
    ...
    def generate_json(
            self, viz_obj, chartname="Chart" ,csv=False, pdf=False, xlsx=False , query=False, results=False, samples=False
    ):
        if csv:
            return CsvResponse(
                viz_obj.get_csv(),
                status=200,
                headers=generate_download_headers("csv", chartname),
        ...
        if pdf:
            return Response(
                viz_obj.get_pdf(),
                status=200,
                headers=generate_download_headers("pdf", chartname),
        ...
        if xlsx:
            return Response(
                viz_obj.get_xlsx(),
                status=200,
                headers=generate_download_headers("xlsx", chartname),
    ...
    def explore_json(self, datasource_type=None, datasource_id=None):
        ...
        invalid_char = {" ",'"',"'"}
        slice_name = "Untitled" if slc == None else ''.join([x for x in slc.slice_name if x not in invalid_char])
        ...
        return self.generate_json(
            viz_obj,chartname=slice_name, csv=csv, xlsx=xlsx, pdf=pdf, query=query, results=results, samples=samples
        )
    ...
    @api
    @expose("/downloadpdf", methods=['POST'])
    def get_pdf(self):
        """
        This api requests:
        1. Origin of the request ex: (https://locahost:8088)
        2. Url of the dashboard
        3. Title of the dashboard (For naming the pdf accordingly)
        4. The css id of the tab which is selected by user

        We process these strings sent by the request and then use them.
        We trigger the nodejs script with command line arguments that it will require to process the pdf

        Text processing of the request are as follows:

        1. We remove double quotes from the url and origin because when the request is send by frontend it comes
        with double quotes at starting and end of the url, string.trim() and string.replace() has been tried to remove
        them but they don't work :-/

        2. We have something called invalid_char, we hunt these characters from the dashboard title as the title is finally going to be
        the name of the pdf so they are removed and finally a formatted name is given to the pdf

        3. We append the current date and time to the PDF

        TODO: Make a cleaner in the scheduler so that it can delete the generated reports every 24 hours
        so that it does not loads up the storage.
        ...
        tab_selected_id = request.form['tab_selected_id']
        tab_selected_id = tab_selected_id[1:-1]
        title = request.form['title']
        dt_string = now.strftime("%d-%b-%Y--%H_%M")
        title += "_" + dt_string
        pdf_name = ''.join([x for x in title if x not in invalid_char])
        #TODO: Make the nodejs script accept command line arguments in more standard way like: node script.js argument1=something argument2=something
        final_command = "node printPdf_singleTab.js {0}  {1} {2} {3}".format(url,pdf_name, tab_selected_id, origin)
        # logging.debug(final_command)
        pdf_path = "/static/reports/" + pdf_name+".pdf"
        pdf_link = origin + pdf_path
        file_name = pdf_name +".pdf"
        try:
            subprocess.run(final_command,shell=True,cwd= app.config['PRINT_PDF_SERVICE'])
            # pdf_file_os_path = "static/reports/" + file_name
            data = {'file_name': pdf_name, 'link': pdf_link}
...
appbuilder.add_view(SubscriberView, "Subscribers", category="Loans", category_label=__("Loans"), category_icon="fa-money",)
appbuilder.add_view(CompanyBrandView, "Company Brands", category="Loans", category_label=__("Loans"), category_icon="fa-money",)
appbuilder.add_view(FulfillmentActionView, "Fulfillment Actions", category="Fulfillment", category_label=__("Fulfillment"), category_icon="fa-handshake-o",)
...
appbuilder.add_view(LoanTypeView, "Loan Types", category="Loans", category_label=__("Loans"), category_icon="fa-money",)
appbuilder.add_view_no_menu(LoanTypeLimitView)#, "Loan Type Limits", category="Loans", category_label=__("Loans"), category_icon="fa-money",)
appbuilder.add_view(NotifyPersonView, "Notify Persons", category="Notifications", category_label=__("Notifications"), category_icon="fa-envelope-open-o",)
appbuilder.add_view_no_menu(NotifyTypeView)#, "Notify Types", category="Notifications", category_label=__("Notifications"), category_icon="fa-envelope-open-o",)
appbuilder.add_view_no_menu(NotifySubscriptionView)#, "Notify Subscriptions", category="Notifications", category_label=__("Notifications"), category_icon="fa-envelope-open-o",)
appbuilder.add_view_no_menu(SegmentView, "Segments")#, category="Loans", category_label=__("Loans"), category_icon="fa-money",)
appbuilder.add_view(IOUBlacklistView, "Blacklist", category="Loans", category_label=__("Loans"), category_icon="fa-money",)

appbuilder.add_view_no_menu(OutstandingLoanView)

appbuilder.add_link(
    "Daily Dashboard",
    label=_("Daily Dashboard"),
...
appbuilder.add_link(
    "NPS Dashboard Daily",
    label=_("NPS Dashboard Daily"),
    href="/superset/dashboard/4",
    category_icon="fa-dashboard",
    category="Dashboards",
    category_label=__("Dashboards"),
)
...
appbuilder.add_link(
    "NPS Dashboard Monthly",
    label=_("NPS Dashboard Monthly"),
    href="/superset/dashboard/5",
    category_icon="fa-dashboard",
    category="Dashboards",
    category_label=__("Dashboards"),
)
...
appbuilder.add_link(
    "Debt Dashboard",
    label=_("Debt Dashboard"),
    href="/superset/dashboard/6",
    category_icon="fa-dashboard",
    category="Dashboards",
    category_label=__("Dashboards"),
)
