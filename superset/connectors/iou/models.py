...
class IOUTableColumn(Model, BaseColumn):

    """ORM object for table columns, each table can have multiple columns"""

    __tablename__ = "iou_table_columns"
    __table_args__ = (UniqueConstraint("table_id", "column_name"),)
    table_id = Column(Integer, ForeignKey("iou_tables.id"))
    table = relationship(
        "IouTable",
    ...
    def get_iou_col(self, label: Optional[str] = None) -> Column:
        label = label or self.column_name
        if not self.expression:
            db_engine_spec = self.table.database.db_engine_spec
            type_ = db_engine_spec.get_iou_column_type(self.type)
            col = column(self.column_name, type_=type_)
        else:
            col = literal_column(self.expression)
        col = self.table.make_iou_column_compatible(col, label)
        return col
    ...
    def get_time_filter(
        ...
        col = self.get_iou_col(label="__time")
        l = []
        if start_dttm:
    ...
    def get_timestamp_expression(
        ...
        if not self.expression and not time_grain and not is_epoch:
            iou_col = column(self.column_name, type_=DateTime)
            return self.table.make_iou_column_compatible(iou_col, label)
        if self.expression:
            col = literal_column(self.expression)
        else:
            col = column(self.column_name)
        time_expr = db.db_engine_spec.get_timestamp_expr(col, pdf, time_grain)
        return self.table.make_iou_column_compatible(time_expr, label)

    @classmethod
    def import_obj(cls, i_column):
        def lookup_obj(lookup_column):
            return (
                db.session.query(IOUTableColumn)
                .filter(
                    IOUTableColumn.table_id == lookup_column.table_id,
                    IOUTableColumn.column_name == lookup_column.column_name,
                )
                .first()
            )
...
IouTable_user = Table(
    ...
    Column("table_id", Integer, ForeignKey("iou_tables.id")),
)


class IouTable(Model, BaseDatasource):
    ...
    column_class = IOUTableColumn
    owner_class = security_manager.user_model

    __tablename__ = "iou_tables"
    ...
    owners = relationship(owner_class, secondary=IouTable_user, backref="iou_tables")
    database = relationship(
        "Database",
        backref=backref("iou_tables", cascade="all, delete-orphan"),
    ...
    def make_iou_column_compatible(
        self, iou_col: Column, label: Optional[str] = None
    ) -> Column:
        """Takes a sql alchemy column object and adds label info if supported by engine.
        :param iou_col: sql alchemy column instance
        :param label: alias/label that column is expected to have
        :return: either a sql alchemy column or label instance if supported by engine
        """
        label_expected = label or iou_col.name
        db_engine_spec = self.database.db_engine_spec
        if db_engine_spec.allows_column_aliases:
            label = db_engine_spec.make_label_compatible(label_expected)
            iou_col = iou_col.label(label)
        iou_col._df_label_expected = label_expected
        return iou_col
    ...
    def get_datasource_by_name(
        ...
        # in python than in the IOU query in a multi-dialect way
    ...
    @property
    def data(self) -> Dict:
        d = super(IouTable, self).data
        if self.type == "table":
            grains = self.database.grains() or []
            if grains:
                grains = [(g.duration, g.name) for g in grains]
            d["granularity_iou"] = utils.choicify(self.dttm_cols)
            d["time_grain_iou"] = grains
    ...
    def get_query_str_extended(self, query_obj: Dict) -> QueryStringExtended:
        iouq = self.get_iou_query(**query_obj)
        sql = self.database.compile_sqla_query(iouq.iou_query)
        logging.info(sql)
        sql = sqlparse.format(sql, reindent=True)
        sql = self.mutate_query_from_config(sql)
        return QueryStringExtended(
            labels_expected=iouq.labels_expected, sql=sql, prequeries=iouq.prequeries
        )
    ...
    def get_from_clause(self, template_processor=None):
        ...
        return self.get_iou_table()

    def adhoc_metric_to_iou(self, metric: Dict, cols: Dict) -> Optional[Column]:
        """
        ...
        if expression_type == utils.ADHOC_METRIC_EXPRESSION_TYPES["SIMPLE"]:
            ...
            if table_column:
                iou_column = table_column.get_iou_col()
            else:
                iou_column = column(column_name)
            iou_metric = self.iou_aggregations[metric["aggregate"]](iou_column)
        elif expression_type == utils.ADHOC_METRIC_EXPRESSION_TYPES["SQL"]:
            iou_metric = literal_column(metric.get("sqlExpression"))
        else:
            return None

        return self.make_iou_column_compatible(iou_metric, label)

    def get_iou_query(  # sqla
        self,
        groupby,
        metrics,
        granularity,
        ...
        for m in metrics:
            if utils.is_adhoc_metric(m):
                metrics_exprs.append(self.adhoc_metric_to_iou(m, cols))
            elif m in metrics_dict:
                metrics_exprs.append(metrics_dict[m].get_iou_col())
            else:
        ...
        if metrics_exprs:
            ...
            main_metric_expr = self.make_iou_column_compatible(main_metric_expr, label)
        ...
        if groupby:
            ...
            for s in groupby:
                if s in cols:
                    outer = cols[s].get_iou_col()
                else:
                    outer = literal_column(f"({s})")
                    outer = self.make_iou_column_compatible(outer, s)
            ...
            for s in columns:
                select_exprs.append(
                    cols[s].get_iou_col()
                    if s in cols
                    else self.make_iou_column_compatible(literal_column(s))
                )
            metrics_exprs = []
        ...
        if granularity:
            ...
            time_grain = extras.get("time_grain_iou")
            time_filters = []
        ...
        for flt in filter:
            ...
            if col_obj:
                ...
                if op in ("in", "not in"):
                    cond = col_obj.get_iou_col().in_(eq)
                    if "<NULL>" in eq:
                        cond = or_(cond, col_obj.get_iou_col() == None)
                    if op == "not in":
                        cond = ~cond
                    ...
                    if op == "==":
                        where_clause_and.append(col_obj.get_iou_col() == eq)
                    elif op == "!=":
                        where_clause_and.append(col_obj.get_iou_col() != eq)
                    elif op == ">":
                        where_clause_and.append(col_obj.get_iou_col() > eq)
                        ...
                        where_clause_and.append(col_obj.get_iou_col().like(eq))
                    elif op == "IS NULL":
                        where_clause_and.append(col_obj.get_iou_col() == None)
                    elif op == "IS NOT NULL":
                        where_clause_and.append(col_obj.get_iou_col() != None)
        if extras:
        ...
        for col, ascending in orderby:
            ...
            if utils.is_adhoc_metric(col):
                col = self.adhoc_metric_to_iou(col, cols)
            elif col in cols:
                col = cols[col].get_iou_col()
        ...
        if is_timeseries and timeseries_limit and groupby and not time_groupby_inline:
            if self.database.db_engine_spec.allows_joins:
                ...
                inner_main_metric_expr = self.make_iou_column_compatible(
                    main_metric_expr, "mme_inner__"
                )
                inner_groupby_exprs = []
                inner_select_exprs = []
                for gby_name, gby_obj in groupby_exprs_sans_timestamp.items():
                    inner = self.make_iou_column_compatible(gby_obj, gby_name + "__")
    ...
    def get_iou_table_object(self) -> Table:
        return self.database.get_table(self.table_name, schema=self.schema)

    def fetch_metadata(self) -> None:
        """Fetches the metadata for the table and merges it in"""
        try:
            table = self.get_iou_table_object()
        except Exception as e:
            logging.exception(e)
            raise Exception(
                _(
        ...
        dbcols = (
            db.session.query(IOUTableColumn)
            .filter(IOUTableColumn.table == self)
            .filter(or_(IOUTableColumn.column_name == col.name for col in table.columns))
        )
        ...
        for col in table.columns:
            ...
            if not dbcol:
                dbcol = IOUTableColumn(column_name=col.name, type=datatype)
    ...
    def import_obj(cls, i_datasource, import_time=None) -> int:
        ...
        def lookup_IouTable(table):
            return (
                db.session.query(IouTable)
                .join(Database)
                .filter(
                    IouTable.table_name == table.table_name,
                    IouTable.schema == table.schema,
        ...
        return import_datasource.import_datasource(
            db.session, i_datasource, lookup_database, lookup_IouTable, import_time
        )
    ...
    def get_extra_cache_keys(self, query_obj: Dict) -> List[Any]:
        if self.has_extra_cache_keys(query_obj):
            iou_query = self.get_iou_query(**query_obj)
            extra_cache_keys = iou_query.extra_cache_keys
            return extra_cache_keys
        return []


sa.event.listen(IouTable, "after_insert", security_manager.set_perm)
sa.event.listen(IouTable, "after_update", security_manager.set_perm)
