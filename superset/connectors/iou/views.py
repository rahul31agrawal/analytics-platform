...
class IOUTableColumnInlineView(CompactCRUDMixin, SupersetModelView):
    datamodel = SQLAInterface(models.IOUTableColumn)
    ...
    add_form_extra_fields = {
        "table": QuerySelectField(
            "Table",
            query_factory=lambda: db.session().query(models.IouTable),
            allow_blank=True,
...
appbuilder.add_view_no_menu(IOUTableColumnInlineView)


class IOUSqlMetricInlineView(CompactCRUDMixin, SupersetModelView):
...
appbuilder.add_view_no_menu(IOUSqlMetricInlineView)


class IOUTableModelView(DatasourceModelView, DeleteMixin, YamlExportMixin):
    datamodel = SQLAInterface(models.IouTable)

    list_title = _("IOU Tables")
    show_title = _("Show IOU Table")
    add_title = _("Import a table definition")
    edit_title = _("Edit IOU Table")
    ...
    related_views = [IOUTableColumnInlineView, IOUSqlMetricInlineView]
    ...
    def pre_add(self, table):
        with db.session.no_autoflush:
            table_query = db.session.query(models.IouTable).filter(
                models.IouTable.table_name == table.table_name,
                models.IouTable.schema == table.schema,
                models.IouTable.database_id == table.database.id,
            )
    ...
    def edit(self, pk):
        ...
        resp = super(IOUTableModelView, self).edit(pk)
    ...
    def refresh(self, tables):
        ...
        return redirect("/ioutablemodelview/list/")


appbuilder.add_view_no_menu(IOUTableModelView)
appbuilder.add_link(
    "IOUTables",
    label=__("IOUTables"),
    href="/ioutablemodelview/list/?_flt_1_is_sqllab_view=y",
    icon="fa-table",
    category="Sources",
