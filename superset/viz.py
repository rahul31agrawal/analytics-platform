...
class BaseViz(object):
    ...
    def get_df(
        self, query_obj: Optional[Dict[str, Any]] = None, limit=False
        ...
        if limit:
            query_obj["row_limit"] = config.get("EXPORT_ROW_LIMIT")
    ...
    def get_width_col(self,col,dataframe):
        """
        Returns the max_width which we want to set for a column, points take in consideration
        are: 1. Max_width limit of a column = 100
             2. Max_width initially considered as the no of characters in the column's header
             3. If any content while iteration is more than the length of header than we update
        """
        max_width = len(col)
        max_limit = 100
        len_df_col = len(dataframe)
        if len_df_col < 0:
            return None
        if len_df_col < 1 and len_df_col > 0:
            max_width = len(str(dataframe[col][0]))
            return max_width
        else:
            row_c = 2
            while(row_c < len_df_col):
                if dataframe[col][row_c] != None:
                    max_w = len(str(dataframe[col][row_c]))
                    if max_w > max_width:
                        max_width = max_w
                row_c *= 4
            if max_limit < max_width:
                return max_limit
            return max_width


    def get_max_widths(self,dataframe):

        cols = dataframe.columns
        widths = []
        for col in cols:
            widths.append(self.get_width_col(col,dataframe))
    ...
    def get_xlsx(self):
        ...
        df.to_excel(writer,index=include_index,merge_cells=False)
        workbook  = writer.book
        worksheet = writer.sheets['Sheet1']
        widths = self.get_max_widths(df)
        for index in range(len(widths)):
            worksheet.set_column(index,index,widths[index]+2)
        writer.close()
        output.seek(0)
    ...
    def get_pdf(self):
        ...
        if no_of_col > config.get("CHART_TO_PDF_COLUMN_LIMIT_A3"):
...
class NVD3DualLineViz(NVD3Viz):
    ...
    def get_chart_data(self, metrics, df, classed, y_axis, bar_cols, dashed_line_col):
        chart_data = []
        series = df.to_dict("series")
        for i, m in enumerate(metrics):
            ...
            style = " ".join([classed, 'dashed']) if series_title == dashed_line_col else classed
            ...
            d = {
                ...
                "yAxis": y_axis,
                "type": ["line", "bar"][series_title in bar_cols],
            }
            chart_data.append(d)
        return chart_data
    ...
    def to_series(self, df, classed=""):
        ...
        series = df.to_dict("series")
        chart_data = []
        bars = self.form_data.get('bars') or []
        bar_cols = [utils.get_metric_name(i) for i in bars]

        dashed_line = self.form_data.get('dashed_line') or ""
        dashed_line_col = utils.get_metric_name(dashed_line)

        m1 = self.form_data.get("metric",)
        m2 = self.form_data.get("metric_2")
        chart_data.extend(self.get_chart_data(m1, df, classed, 1, bar_cols, dashed_line_col))
        chart_data.extend(self.get_chart_data(m2, df, classed, 2, bar_cols, dashed_line_col))
        return chart_data
    ...
    def get_data(self, df):
        ...
        metrics = metric + metric_2
        metrics = [utils.get_metric_name(i) for i in metrics]
        df = df.pivot_table(index=DTTM_ALIAS, values=metrics)
...
class GroupedColumnLineViz(BaseViz):

    """Grouped Column Line Chart"""

    viz_type = "grouped_column_line_chart"
    verbose_name = _("Grouped Column Line Chart")
    is_timeseries = False
    ...
    def query_obj(self):
        ...
        numeric_columns = self.form_data.get("all_columns_x") or []
        metrics = self.metric_labels
        if metrics is None:
            raise Exception(_("Must have at least one Metric specified"))
        self.columns = numeric_columns + metrics
        # d["columns"] = numeric_columns + metrics + [self.form_data.get("series")]
        # override groupby entry to avoid aggregation
        d["groupby"] = [self.form_data.get("series")]
        return d
    ...
    def get_data(self, df):
        ...
        if len(metrics) > 0:
            ...
            raise Exception(_("No Valid Columns to fetch data for"))

        all_keys = list(data.keys())
        for i in range(1, len(all_keys)):
            chart_data.append({
                "key": all_keys[i],
                "renderas": "bar" if i == 1 else "line",
                "values": [{"x": data[all_keys[0]][j], "y": data[all_keys[i]][j]} for j in range(len(data[all_keys[i]]))]
            })

        return chart_data


class GroupedStackedBarViz(BaseViz):

    """ Grouped Stacked Bar Chart """
    viz_type = "grouped_stacked_bar_chart"
    verbose_name = _("Grouped Stacked Bar chart")
    is_timeseries = False
    ...
    def query_obj(self):
        ...
        # d["columns"] = numeric_columns + metrics + [self.form_data.get("series")]
        # override groupby entry to avoid aggregation
        d["groupby"] = [self.form_data.get("grouping_column")]
        return d
