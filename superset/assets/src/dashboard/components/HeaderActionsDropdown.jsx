...
class HeaderActionsDropdown extends React.PureComponent {
  ...
  constructor(props) {
    ...
    this.download_pdf_api = this.download_pdf_api.bind(this);
  ...
  download_pdf_api () {
    let tab_selected_id = "Null";
    if (document.getElementsByClassName("dashboard-component dashboard-component-tabs").length !== 0) {
      let tabs = document.querySelectorAll('*[id^="TABS-"]');
      // let to_return = [];
      let id_tabs = [];

      for(let i = 0; i<tabs.length; i++) {

        id_tabs.push(tabs[i].id.match(/TABS-(\w{10})(?!-)/g));
      }
      let containerdiv = document.getElementById(id_tabs[0]);
      let tab_list = containerdiv.getElementsByClassName("active");
      let element = tab_list[0].getElementsByTagName("a");
      tab_selected_id = element[0].id;
    }
    this.props.addSuccessToast(
      t('You\'re Report is being Generated. Please do not refresh or close the tab!')
    );
    SupersetClient.post ({
      endpoint: `/superset/downloadpdf`,
      postPayload: {
        "origin":window.location.origin,
        "url":window.location.href,
        "title": document.title,
        "tab_selected_id": tab_selected_id
      },
    }).then(response => {
      const url = response.json.link;
      this.props.addSuccessToast(
        t('Now Downloading...')
      );
      download_file(url, response.json.file_name);
    }).catch(resp => {
      this.props.addDangerToast(
        t('Some Error Occurred! We\'re on it!!')
      )
    });
  ...
  render() {
    ...
    return (
      ...
      >
        ...
        <MenuItem onClick={this.download_pdf_api} disabled={isLoading}>
          {t('Export to pdf')}
        </MenuItem>
